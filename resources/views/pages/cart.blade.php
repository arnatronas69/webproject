  ...
  <body>
                      ...
                      <tr>
                        <th class="ps-0 py-3 border-light" scope="row">
                          <div class="d-flex align-items-center">
                            <a class="reset-anchor d-block animsition-link" href="{{ route('product.show', ['id' => $cartItem->product->id]) }}">
                              <img src="{{ $cartItem->product->image }}" alt="{{ $cartItem->product->name }}" width="70" />
                            </a>
                            <div class="ms-3">
                              <strong class="h6">
                                <a class="reset-anchor animsition-link" href="{{ route('product.show', ['id' => $cartItem->product->id]) }}">
                        ...
                        <td class="p-3 align-middle border-light">
                          <p class="mb-0 small">${{ $cartItem->product->price }}</p>
                        </td>
                        <td class="p-3 align-middle border-light">
                          <div class="border d-flex align-items-center justify-content-between px-3">
                            ...
                            <div class="quantity">
                              <form action="{{ route('cart.updateQuantity', ['id' => $cartItem->id]) }}" method="POST">
                                @csrf
                                <!-- Use _method to simulate PATCH method -->
                                <input type="hidden" name="_method" value="PATCH">
                                <!-- Form fields and submit button -->
                                <button class="dec-btn p-0" type="button"><i class="fas fa-caret-left"></i></button>
                                <input class="form-control form-control-sm border-0 shadow-0 p-0 quantity-input" type="number" name="quantity" value="{{ $cartItem->quantity }}" min="1" data-cart-item-id="{{ $cartItem->id }}">
                        ...
                        <td class="p-3 align-middle border-light">
                          <form action="{{ route('cart.remove', ['id' => $cartItem->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="reset-anchor border-0 bg-transparent">
                  ...
                  <div class="col-md-6 text-md-end">
                    <div class="col-md-6 text-md-end">
                      <form action="{{ route('checkout.process') }}" method="POST">
                        @csrf
                        <input type="hidden" name="names" value="{{ urlencode(implode(',', $productNames)) }}">
                        <input type="hidden" name="prices" value="{{ urlencode(implode(',', $productPrices)) }}">
                        <input type="hidden" name="total" value="{{ $total }}">
                        <button type="submit" class="btn btn-outline-dark btn-sm">Proceed to Checkout</button>
                      </form>
                      </div>
                  </div>                    
              </div>
                ...
                <div class="card-body">
                  ...
                  <ul class="list-unstyled mb-0">
                    <li class="d-flex align-items-center justify-content-between">
                      ...
                      <span id="subtotal" class="text-muted small">${{ $subtotal }}</span>
                    </li>
    ...
    <script>
      function updateCart(input) {
        ...
        var previousSubtotal = parseFloat(subtotalElement.innerText.slice(1)); // Remove the dollar sign and parse as a float
        var previousTotal = parseFloat(totalElement.innerText.slice(1)); // Remove the dollar sign and parse as a float
        var total = (previousTotal - previousSubtotal + subtotal) ;
        ...
        var url = '/cart/' + cartItemId + '/update-quantity';
        var data = {
          quantity: quantity
        };
    ...
    <script>
      $(document).ready(function() {
          ...
          function updateQuantity(productId, quantity) {
              $.ajax({
                  url: '/cart/' + productId + '/update-quantity',
                  type: 'PATCH',
                  data: {
                      _token: '{{ csrf_token() }}',
                      quantity: quantity
